...
func TestPlaybackAndChangeBus(t *testing.T) {
	...
	pb := lib.Playback()
	if _, err := pb.SetStar(ctx, "", item, true, nil); err != nil {
		t.Fatalf("star: %v", err)
	}
	r := 90
...
func TestStatsOnFacet(t *testing.T) {
	...
	_, _ = pb.SetStar(ctx, "", items[0].PID, true, nil)
...
func TestLogicalExport(t *testing.T) {
	...
	items, _ := lib.Query(ctx, query.New(query.EntityItems).Build(), "")
	if _, err := lib.Playback().SetStar(ctx, "", items[0].PID, true, nil); err != nil {
		t.Fatalf("star: %v", err)
	}
	r := 85
	if _, err := lib.Playback().SetRating(ctx, "", items[0].PID, &r, nil); err != nil {
		t.Fatalf("rate: %v", err)
	}
