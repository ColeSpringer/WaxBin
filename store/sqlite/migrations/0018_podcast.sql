-- WaxBin schema v18: podcasts.
-- A podcast is a subscribed feed; an episode is a playable_item (kind='episode')
-- under it, mirroring how a track/book shares the supertype PK. Episodes are
-- cataloged from the feed with state='remote' (known but not local) and gain a
-- file when downloaded (state='present'); retention drops the downloaded file and
-- returns the episode to 'remote' while keeping its play_state (the play_state FK
-- is to the item, which survives). A feed's identity is its URL or its
-- <podcast:guid>, never its title, so a retitled show keeps its pid and episodes.
-- A truncated feed (older items dropped) never deletes the episodes it stopped
-- listing: upserts add and update, they do not prune.
...
CREATE TABLE podcast (
  ...
  -- HTTP conditional-GET validators so an unchanged feed answers 304 and costs no
  -- bytes on the next sync.
  etag            TEXT    NOT NULL DEFAULT '',
  last_modified   TEXT    NOT NULL DEFAULT '',
  last_fetched_at INTEGER,                          -- unix ns of last successful sync
  -- Retention: keep the newest N downloaded episodes (0 = keep all). Enforced via
  -- the prune delete path so reclaimed space bypasses the trash.
...
-- mirroring track/book. The enclosure_* columns are the remote media pointer that
-- survives a download/retention cycle; a downloaded episode also has a file row
-- linked through item_file (role='primary').
...
-- Stored transcript text per episode. The writer keeps transcript_fts (declared in
-- v2 with no producer until now) in sync inside the same transaction, so a body
-- hit can be ranked below a title hit at search time.
