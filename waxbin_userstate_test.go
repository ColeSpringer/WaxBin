...
func TestUserStateQueryThroughFacade(t *testing.T) {
	...
	pb := lib.Playback()
	if _, err := pb.SetStar(ctx, "", byTitle["A"], true, nil); err != nil {
		t.Fatal(err)
	}
	r90 := 90
	if _, err := pb.SetRating(ctx, "", byTitle["A"], &r90, nil); err != nil {
		t.Fatal(err)
	}
	if _, err := pb.SetStar(ctx, bob.PID, byTitle["B"], true, nil); err != nil {
		t.Fatal(err)
	}
...
func TestPlayStatesForItemsFacade(t *testing.T) {
	...
	pb := lib.Playback()
	if _, err := pb.SetStar(ctx, def.PID, byTitle["A"], true, nil); err != nil {
		t.Fatal(err)
	}
	if _, err := pb.SetStar(ctx, bob.PID, byTitle["A"], true, nil); err != nil {
		t.Fatal(err)
	}
	r := 90
	if _, err := pb.SetRating(ctx, bob.PID, byTitle["B"], &r, nil); err != nil {
		t.Fatal(err)
	}
